...
$builddep -q -y python-wradlib.spec

if [[ $image =~ ^fedora: || $image =~ ^centos: ]]
then
    mkdir -p ~/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
    cp python-wradlib.spec ~/rpmbuild/SPECS/
    spectool -g -R -S ~/rpmbuild/SPECS/python-wradlib.spec
    set +x
    rpmbuild -ba ~/rpmbuild/SPECS/python-wradlib.spec 2>&1 | pv -q -L 3k
else
